...
{
	printf("\n********TESTING CARD EFFECT GREAT HALL********\n");
	...
	cardEffect(great_hall, choice1, choice2, choice3, &testCase, handPos, &bonus);

	player = whoseTurn(&testCase);

	printf("\nTESTING HAND COUNT\n");
	if (testCase.handCount[player] == game.handCount[player])
		printf("\tTEST PASSED");
	else
		printf("\tTEST FAILED");

	printf("\nTESTING ACTION COUNT\n");
	if (testCase.numActions - 1 == game.numActions)
		printf("\tTEST PASSED");
	else
		printf("\tTEST FAILED");
	...
	printf("\n********END TESTING CARD EFFECT GREAT HALL********\n");
}
